...
def get_audio_stream_info(path: str) -> Tuple[Optional[dict], Optional[str]]:
    try:
        ...
        command = [
            ...
            '-print_format', 'json',
            '-show_streams',
            '-select_streams', 'a:0',  # Select only the first audio stream
            '-i', path
        ]
        ...
        if audio_streams:
            return audio_streams[0], None  # Return the first audio stream's info dict
        
        return None, "ファイル内に音声ストリームが見つかりませんでした。"
    except FileNotFoundError:
        return None, "ffprobeが見つかりません。FFmpegが正しくインストールされているか確認してください。"
    except (subprocess.CalledProcessError, json.JSONDecodeError, TypeError, ValueError, OSError) as e:
        return None, f"ffprobeによる音声情報の取得に失敗しました: {e}"
